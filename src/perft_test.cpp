...
struct KnownPos {
    ...
    int depth;
    NodeCount count;
    constexpr KnownPos(int depth, NodeCount count)
        : fen(fen::initialPosition), depth(depth), count(count) {}
    constexpr KnownPos(std::string_view fen, DepthAndCount depthAndCount)
        : fen(fen), depth(depthAndCount.first), count(depthAndCount.second) {}
};

constexpr KnownPos twoKings = {"4k3/8/8/8/8/8/8/4K3 w - - 0 1",
                               kDebug ? DepthAndCount{8, 4'656'573}
                                      : DepthAndCount{12, 10'109'930'485}};
constexpr KnownPos maxMoves = {"R6R/3Q4/1Q4Q1/4Q3/2Q4Q/Q4Q2/pp1Q4/kBNN1KB1 w - - 0 1",
                               kDebug ? DepthAndCount{3, 19'073} : DepthAndCount{5, 13'853'661}};
constexpr KnownPos twoKingsRook = {"4k3/8/8/8/8/8/8/4Kr2 w - - 0 1",
                                   kDebug ? DepthAndCount{5, 21'570} : DepthAndCount{7, 2'362'837}};
constexpr KnownPos twoKingsOnly = {"4k3/8/8/8/8/8/8/4K3 w - - 0 1",
                                   kDebug ? DepthAndCount{7, 375'049}
                                          : DepthAndCount{10, 122'753'895}};
...
void testKnownPositions() {
    ...
    if (options::cachePerft) {
        knownPositions.emplace_back(
            Known{"4k3/8/8/8/8/8/8/4Kr2 w - - 0 1", twoKingsRook.depth, twoKingsRook.count});
        knownPositions.emplace_back(
            Known{"4k3/8/8/8/8/8/8/4K3 w - - 0 1", twoKingsOnly.depth, twoKingsOnly.count});
    }
...
void testStartingPosition(int depth) {
    ...
    std::vector<KnownPos> knownPositions = {{1, "20"_u128},
                                            {2, "400"_u128},
                                            {3, "8'902"_u128},
                                            {4, "197'281"_u128},
                                            {5, "4'865'609"_u128},
...
void testTwoKings() {
    ...
    NodeCount count = perft(position, twoKings.depth);
    if (count != twoKings.count)
        error("Expected " + to_string(twoKings.count) + " nodes for position '" + fen +
              "' at depth " + std::to_string(twoKings.depth) + ", but got " + to_string(count));
}
...
void testMaxMoves() {
    ...
    NodeCount count = perft(position, maxMoves.depth);
    if (count != maxMoves.count)
        error("Expected " + to_string(maxMoves.count) + " nodes for position '" + fen +
              "' at depth " + std::to_string(maxMoves.depth) + ", but got " + to_string(count));
}
