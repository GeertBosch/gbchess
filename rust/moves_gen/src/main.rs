use fen::{parse_piece_placement, parse_position, position_to_string, Board, CastlingMask, Color, Piece, Square, Turn, NO_EN_PASSANT_TARGET};
...
fn run_basic_tests() {
    ...
    // Test 1: Basic pawn moves
    {
        ...
        let turn = Turn::new(Color::White, CastlingMask::EMPTY, NO_EN_PASSANT_TARGET, 0, 0);
    ...
    // Test 3: Simple board with pieces
    {
        if let Ok(board) = parse_piece_placement("r3kbnr/pP1qpppp/3p4/4N3/4P3/8/PPP2PPP/RNB1K2R") {
            let turn = Turn::new(Color::White, CastlingMask::KQ_kq, NO_EN_PASSANT_TARGET, 0, 0);
